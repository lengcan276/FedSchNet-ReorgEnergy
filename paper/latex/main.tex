...
\begin{enumerate}
  ...
  \item \textbf{Calibration generalisation is regime-dependent.} Additional controlled pseudo-federation experiments, reported in the Supporting Information (\S S11), support the calibration-centered interpretation in small-target pseudo-tasks: task-specific calibration improved FedPer on 9 of 10 label-quantile pseudo-targets at $n = 50$ (median $\Delta$MAE $= -0.020$~eV; paired Wilcoxon $p < 0.001$ on 7 of 10 tasks). However, an aggressive label-scale stress test at $n_{\mathrm{target}} = 100$ did not show an advantage of calibration over plain FedPer ($\Delta$MAE $= +0.010$~eV, paired Wilcoxon $p = 0.73$), indicating that the calibration benefit observed on the real C-triplet target is regime-dependent rather than universal.
\end{enumerate}
